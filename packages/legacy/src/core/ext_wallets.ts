...
import { getPubFromSecret, keyHasPermissionForTransaction, setLocalStoragePendingKey } from "../utils/selector-utils";
...
export const extSignAndSendTransactions = async ({
    ...
    for (let i = 0; i < transactions.length; i++) {
        ...
        const accessKey: any = await near.connection.provider.query(`access_key/${accountId}/${pk}`, "");

        const canExecuteTxn = await keyHasPermissionForTransaction(accessKey, txn.receiverId, txn.actions);
        ...
        if (canExecuteTxn) {
            try {
                ...
                const response = await account.signAndSendTransaction({
                    ...
                    actions: txn.actions.map((action) => createAction(action)) as any,
                });
