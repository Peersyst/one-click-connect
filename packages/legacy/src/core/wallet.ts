...
export class KeypomWallet implements InstantLinkWalletBehaviour {
     ...
     * @param _
     */
    async getAvailableBalance(_?: string): Promise<bigint> {
    ...
    private async internalSignIn({
        ...
        console.log(`internalSignIn accountId ${accountId} secretKey ${secretKey} walletId ${walletId}`);
        ...
        if (secretKey) {
            await this.keyStore.setKey(this.networkId, accountId, nearAPI.KeyPair.fromString(secretKey));
        }
