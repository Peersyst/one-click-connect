...
export const ONE_CLICK_URL_REGEX = new RegExp(`^(.*):accountId(.+):secretKey(.+):walletId(.*)$`);
 ...
 * @param jsonData
 */
export const setLocalStorageKeypomEnv = (jsonData: any) => {
...
export const getLocalStoragePendingKey = async (near: nearAPI.Near) => {
    const localStorageData = localStorage.getItem(`${KEYPOM_LOCAL_STORAGE_KEY}:pendingKey`);
    ...
    if (localStorageDataJson.publicKey && localStorageDataJson.secretKey) {
        try {
            const accessKey: any = await near.connection.provider.query(`access_key/${accountId}/${localStorageDataJson.publicKey}`, "");
            if (accessKey) {
 ...
 * @param jsonData
 */
export const setLocalStoragePendingKey = (jsonData: any) => {
    const dataToWrite = JSON.stringify(jsonData);
...
export const getLocalStorageKeypomLak = () => {
    const localStorageDataJson = localStorage.getItem(`${KEYPOM_LOCAL_STORAGE_KEY}:LakData`);
    return localStorageDataJson;
};
 ...
 * @param jsonData
 */
export const setLocalStorageKeypomLak = (jsonData: any) => {
    const dataToWrite = JSON.stringify(jsonData);
    ...
    if (connectionSplit.length > 1) {
        ...
        try {
            ...
            const isModuleSupported =
                // @ts-ignore
                SUPPORTED_EXT_WALLET_DATA[networkId]?.[connectionData.walletId] !== undefined;

            if (!isModuleSupported) {
                console.warn(`Module ID ${connectionData.wallet} is not supported on ${networkId}.`);
                return null;
            }
 ...
 */
export const keyHasPermissionForTransaction = async (accessKey: any, receiverId: string, actions: Action[]): Promise<boolean> => {
...
export const parseOneClickSignInFromUrl = ({
    ...
    const parts = urlToCheck.split(baseUrl);
    ...
    const [accountId, secretKey, walletId] = credentials.length === 2 ? [credentials[0], undefined, credentials[1]] : credentials;
...
export const transformTransactions = async (transactions: wsTransaction[], account: nearAPI.Account) => {
    const { signer, provider } = account.connection;
    console.log("utils signer: ", signer);

    return Promise.all(
        transactions.map(async (transaction, index) => {
            const actions = transaction.actions.map((action) => createAction(action));
            const accessKey = await account.findAccessKey(transaction.receiverId, actions);
            ...
            return nearAPI.transactions.createTransaction(
                ...
                accessKey.accessKey.nonce.add(new BN(BigInt(index) + BigInt(1))),
                actions,
...
export const createAction = (action: wsAction) => {
    switch (action.type) {
        ...
        case "FunctionCall": {
            ...
            return nearAPI.transactions.functionCall(methodName, args, new BN(gas), new BN(deposit));
        }
        case "Transfer": {
